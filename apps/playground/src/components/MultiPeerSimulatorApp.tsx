    ...
    () =>
      new Map(
        sessions.map((
          s,
          i,
        ) => [s.peerId, PEER_COLORS[i % PEER_COLORS.length]!]),
      ),
    [sessions],
  );
  ...
  function handleSync(action: string) {
    switch (action) {
      case "sync-all":
        ...
        break;
      case "ab":
        syncService.sync(sessions[0]!, sessions[1]!);
        break;
      case "bc":
        syncService.sync(sessions[1]!, sessions[2]!);
        break;
      case "ac":
        syncService.sync(sessions[0]!, sessions[2]!);
        break;
      case "a-to-b":
        syncService.push(sessions[0]!, sessions[1]!);
        break;
      case "b-to-a":
        syncService.push(sessions[1]!, sessions[0]!);
        break;
    }
    refresh();
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          style={{ ...btnStyle("#c50f1f"), marginLeft: 8 }}
          onClick={handleReset}
        >
          Reset
        </button>
