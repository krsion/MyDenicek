...
export function EventGraphView(
  { events, selectedId, onSelect, peerColorMap }: Props,
) {
  ...
  useEffect(() => {
    ...
    const peerColor = (peer: string) =>
      peerColorMap.get(peer) ?? FALLBACK_COLOR;

    const maxX = Math.max(...nodes.map((n) => n.x)) + PADDING + NODE_RADIUS;
    const maxY = Math.max(...nodes.map((n) => n.y)) + PADDING + NODE_RADIUS +
      30;
    ...
    g.selectAll<SVGLineElement, LayoutLink>("line.edge")
      ...
      .join("line")
      .attr("class", "edge")
      .attr("x1", (d: LayoutLink) => d.source.x)
      .attr("y1", (d: LayoutLink) => d.source.y + NODE_RADIUS)
      .attr("x2", (d: LayoutLink) => d.target.x)
      .attr("y2", (d: LayoutLink) => d.target.y - NODE_RADIUS)
      .attr("stroke", "#bbb")
    ...
    const nodeGroup = g.selectAll<SVGGElement, LayoutNode>("g.node")
      ...
      .on(
        "click",
        (_event: MouseEvent, d: LayoutNode) =>
    ...
    nodeGroup.append("circle")
      ...
      .attr(
        "fill",
        (d: LayoutNode) => d.id === selectedId ? peerColor(d.peer) : "#fff",
      )
      .attr("stroke", (d: LayoutNode) => peerColor(d.peer))
      .attr("stroke-width", 2);
    ...
    nodeGroup.append("text")
      ...
      .attr("font-size", "10px")
      .attr("fill", "#666")
      .text((d: LayoutNode) => d.editKind.replace(/Edit$/, ""));
  ...
  return (
    ...
    >
      <svg
        ref={svgRef}
        style={{ display: "block", minWidth: 200, minHeight: 100 }}
      />
    </div>
  );
}
