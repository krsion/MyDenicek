...
\textbf{FR-07: Recording} [Core] The system shall automatically record all local document mutations by subscribing to Loro's event system. Each local event carries diffs that are accumulated and converted to generalized patches using variable node IDs (\texttt{\$0}, \texttt{\$1}, etc.) that can be bound to different nodes during replay.
...
Raw Loro diffs reference concrete container and tree IDs, tying them to a specific document state. Generalized patches introduce two abstractions on top of Loro diffs that make them portable and replayable: (1)~variable placeholders (\texttt{\$0}, \texttt{\$1}, ...) replace concrete node IDs so the same script can be replayed on different target nodes, and (2)~create patches distinguish copies (\texttt{sourceId}) from direct creates (\texttt{data}) so replay knows whether to call \texttt{copyNode} or \texttt{addChildren}. There are three patch types:
...
\item \texttt{getHistory()} converts accumulated diffs to generalized patches, assigning variable IDs (\texttt{\$1}, \texttt{\$2}, ...) to created nodes and suppressing redundant map/text patches for same-event node creates
\item Select specific actions from history (or use all)
\item Click ``Apply'' to replay on the selected node, or ``Add to Button'' to store in an action node
\end{enumerate}
...
The ``+1'' button records adding a \texttt{formula(sum)} node with the original value moved as a child argument along with \texttt{"1"}. Clicking transforms \texttt{"5"} into a formula \texttt{sum(ref, "1")} that evaluates to \texttt{"6"}.
...
To create the button: select the \texttt{ul}, add a new \texttt{li} with checkbox and text value, then select the recorded actions and click ``Add to Button''. Each button click replays the pattern, appending a new item.
