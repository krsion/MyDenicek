...
> PDF version: [documents/specification/specification.pdf](../documents/specification/specification.pdf)
...
*MyWebnicek* is a web-based programming system built on top of *MyDenicek*, providing a user interface for document navigation, editing, and collaborative features. It demonstrates the end-user programming capabilities through formative examples such as counter apps, todo lists, and document refactoring scenarios. The application is deployed at <https://krsion.github.io/MyDenicek/> with real-time collaboration enabled via a WebSocket sync server hosted on Azure App Service.
...
The project maintains a clear separation between the CRDT substrate (`@mydenicek/core`) and the user interface (`mywebnicek`), enabling future development of alternative front-ends similar to how the original Denicek supports both Webnicek and Datnicek.
...
3. **Document Editing:** Users perform edit operations through toolbar controls. Operations include adding nodes, editing values, moving nodes, and deleting nodes.
...
**FR-01: Document Representation** [Core] The system shall represent documents as trees of nodes, where each node has a unique ID, a kind (element, value, formula, ref, or action), and additional properties based on its kind.
...
**FR-03: Value Nodes** [Core] Value nodes shall contain a text string stored internally as a `LoroText` CRDT container, enabling character-level collaborative editing through splice operations.
...
- Concurrent move operations on the same node shall result in the node appearing in one location (determined by Loro's tree CRDT semantics)
...
**FR-07: Recording** [Core] The system shall automatically record all local document mutations as generalized patches with concrete node IDs. Variable placeholders (`$0`, `$1`, etc.) are assigned only when storing actions in a button or at replay time, not in the recorded history.
...
**FR-09: Formula Nodes** [Core] Formula nodes compute values from an `operation` and child arguments (value nodes, ref nodes, or nested formulas). Evaluation detects circular references and enforces max recursion depth.

**FR-10: Formula Operations** [Core] Built-in operations: string (`concat`, `uppercase`, etc.), math (`sum`, `product`, etc.), and tree (`countChildren`).

**FR-11: Ref Nodes** [Core] Ref nodes reference other nodes via a `target` ID. In formulas, they resolve to the target's value.

**FR-12: Action Nodes** [Core] Action nodes store recorded operations as generalized patches. They have a `label`, `params` (named parameter bindings), and `actions` list. Triggering replays patches with the bound parameters.

#### Web Application Requirements

**FR-13: Document Rendering** [Web] The application shall render the document tree as HTML, allowing users to see the final document appearance.

**FR-14: Node Selection** [Web] Users shall be able to select nodes by clicking on them in the rendered view or navigating with keyboard shortcuts (arrow keys, Tab).

**FR-15: JSON Inspector** [Web] The application shall display a JSON view of the document structure, highlighting the currently selected node.

**FR-16: Element Details Panel** [Web] When an element node is selected, the application shall display its tag, attributes, and available operations.

**FR-17: Add Node Interface** [Web] Users shall be able to add child nodes through a popover interface that allows specifying the tag name or text content.

**FR-18: Recording Controls** [Web] The application shall provide controls to start and stop recording, and display the recorded script.

**FR-19: Replay Controls** [Web] The application shall provide controls to replay a recorded script on the currently selected node.

**FR-20: Keyboard Shortcuts** [Web] The application shall support keyboard shortcuts for common operations:
...
**FR-21: WebSocket Synchronization** [Web] The application shall synchronize document changes with other clients via WebSocket connection to a sync server.

**FR-22: Formula Rendering** [Web] Formula nodes display computed results or formula structure (for debugging).

**FR-23: Action Node Rendering** [Web] Action nodes render as buttons; clicking triggers replay with the bound parameters.
...
**NFR-10: Deployment** The web application shall be deployed to GitHub Pages at <https://krsion.github.io/MyDenicek/>. The sync server shall be deployed to Azure App Service at `wss://mydenicek-sync-prod.azurewebsites.net`, enabling real-time collaboration between users.
...
The core library (`@mydenicek/core`) follows a "deep module" design where **DenicekDocument** is the single entry point, hiding all Loro CRDT complexity behind a simple API:

- Read: `getNode()`, `getParentId()`, `getChildIds()`, `getRootId()`
- Write: `addChild()`, `deleteNode()`, `moveNode()`, `copyNode()`, `updateAttribute()`, `updateTag()`, `spliceValue()`, `updateFormulaOperation()`, `updateRefTarget()`
...
**Formula Nodes** compute values. They have an `operation` attribute (e.g., "sum") and children serving as arguments.
...
**String:** `lowerText`, `upperText`, `capitalize`, `concat` (variadic), `trim`, `length`, `replace`, `splitFirst`, `splitLast`
...
An action node stores: `label` (display text), `params` (named parameter bindings mapping names to node IDs), and `actions` (list of `GeneralizedPatch` objects).

#### Generalized Patches

Generalized patches are portable representations of document mutations with two key abstractions: (1) named variable placeholders (`$wrapper`, `$innerValue`, etc.) can replace concrete node IDs so the same script can be replayed on different target nodes, and (2) create patches distinguish copies (`sourceId`) from direct creates (`data`) so replay knows whether to call `copyNode` or `addChildren`.
...
Variables: Named parameters (`$wrapper`, `$innerValue`, etc.) reference existing nodes via the action node's `params` mapping; numeric variables (`$1`, `$2`, ...) are assigned to nodes created during replay.

#### Recording

Recording is automatic — each mutation method in `DenicekDocument` records its operation as a generalized patch:
...
4. The UI displays patches with clickable node IDs; users can retarget any ID to a different node and select a subset of actions
5. Click "Apply" to replay on the selected node, or "Add to Button" to store in an action node — at this point, concrete IDs are generalized to named variable placeholders (`$wrapper`, `$innerValue`, etc.) automatically

#### Replay

1. Click button or use replay controls
2. System builds variable map from the action node's `params` (e.g., `$wrapper` → node ID)
3. For each patch: substitute variables, apply patch, assign new numeric variables (`$1`, `$2`, ...) to created nodes
...
Demonstrates formulas and action buttons. The document contains a `formula(sum)` node displaying the current count and two action buttons ("Increment" and "Decrement").

To create the increment button: the user adds a value node `"1"` as a child of the sum formula and records this step. The recorded action is programmed to the "Increment" action button. To create the decrement button: the user adds a value node `"-1"` as a child and programs the step to the "Decrement" button.

Since `sum` is variadic, each button click appends another argument to the same formula. After several clicks the formula looks like `sum(1, -1, 1, 1, 1, -1)` and evaluates to the running total.

#### Todo List

Demonstrates recording a multi-step "add item" pattern. The document contains a `ul` with existing `li` items, a text input for the new item name, and an "Add" action button targeting the list.

To create the button: the user selects the `ul`, adds a new empty `li`, then copies the input value into the new item's text. The user records both steps and programs them to the "Add" action button. Each button click replays the pattern, appending a new item with the current input text. Finished todo items can be removed by selecting and deleting them.

#### Price Calculator (Formulas)

Demonstrates reactive computation. The document contains value nodes for quantity (`"3"`) and price (`"25"`), plus a `formula(product)` node referencing both. The formula evaluates to `"75"` and updates automatically when either value changes.
...
**Recording the transformation:** The user performs the following steps on the first row:
...
5. Add a `formula(splitFirst)` node as a sibling, with a `ref` pointing to the value node — this displays "John"
6. Add a second `td` element to the `tr`
7. Add a `formula(splitLast)` node inside the second `td`, with a `ref` pointing to the same value node — this displays "Doe"
...
**Creating the action button:** The user selects the recorded steps (excluding the one-time `ul` → `table` rename) and clicks "Add to Button". The recording references two node IDs. For each referenced ID, the user can either mark it as *fixed* (hardcoded) or give it a name to make it a parameter:
...
This workflow is similar to how `ref` node targets are selected, providing a consistent interaction pattern.
...
**Formative Examples** Evaluation against examples from the Denicek paper (Section 2.7): counter (reactive values, action buttons), todo list (recording patterns), price calculator (formula engine).
