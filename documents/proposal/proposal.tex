...
\emph{Denicek} \cite{denicek2024} is a document-based end-user programming substrate that can be used as the basis for the implementation of different programming systems. One such programming system is the web-based system \emph{Webnicek}. Internally, \emph{Denicek} relies on synchronization between document versions and it currently uses Operational Transformation, which is error prone and complex and requires a central server for synchronization. This project will create \emph{MyDenicek}, a backend built on top of CRDTs. This will make it easier to implement programming systems that follow the principles of local-first software. \emph{MyWebnicek} will be a new web-based programming system built on top of the new substrate, providing functionality similar to \emph{Webnicek}.
...
The primary deliverable of the project will be the CRDT-based backend, \emph{MyDenicek}, implemented as a reusable library. \emph{MyWebnicek} will be developed as a prototype/demo application built on top of \emph{MyDenicek}; it is intended as an illustrative demo rather than a fully production-ready web application.
...
\subsection{MyDenicek functional requirements}
The core library (\textit{MyDenicek}) will satisfy the following requirements:
\begin{itemize}
\setlength{\itemsep}{-5pt}
    \item Provide a way of representing HTML-like structured documents similar to those in Denicek
    \item Expose an API for modifying the underlying document through edit actions similar to those in Denicek
    \item Provide a CRDT-based operation for merging divergent versions of the document
    \item Support a mechanism for detecting and resolving merge conflicts akin to Grove
\end{itemize}
...
\end{thebibliography}
\appendix
\section{MyDenicek document structure}
A \emph{MyDenicek document} is represented via a \texttt{DocumentView} class that encapsulates the tree structure, using \textbf{Loro CRDTs}~\cite{loro2024} for synchronization:
...
Internally, the document is stored as a \texttt{LoroTree}---Loro's native movable tree CRDT that handles concurrent structural edits, move operations, and conflict resolution. The \texttt{DocumentView} class is the \emph{public API} that:
\begin{itemize}
\setlength{\itemsep}{-5pt}
...
Conflict resolution uses deterministic wrapper IDs (\texttt{w-\$\{nodeId\}}) for wrap operations and Last-Writer-Wins (LWW) semantics for concurrent tag/value edits.
