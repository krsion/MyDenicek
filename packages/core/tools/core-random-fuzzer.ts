...
type R = () => number;
const int = (r: R, a: number, b: number) =>
  randomIntegerBetween(a, b, { prng: r });
...
const isRec = (n: unknown): n is Obj =>
  n !== null && typeof n === "object" && !("$items" in (n as Obj));
const isLst = (n: unknown): n is Obj =>
  n !== null && typeof n === "object" && "$items" in (n as Obj);
...
function resolveAll(node: unknown, segs: string[], i: number): unknown[] {
  ...
  const s = segs[i]!;
  if (s === "*" && isLst(node)) {
    return lst(node).flatMap((it) => resolveAll(it, segs, i + 1));
  }
  if (isLst(node)) {
    const idx = Number(s);
    return idx >= 0 && idx < lst(node).length
      ? resolveAll(lst(node)[idx], segs, i + 1)
      : [];
  }
...
function applyRandomEdit(
  ...
  const newItem = (): PlainNode =>
    weighted<PlainNode>(r, [
      ...
      {
        $tag: pick(r, TAGS),
        $items: [{ $tag: pick(r, TAGS), name: pick(r, VALS) }, pick(r, VALS)],
      } as PlainNode,
      pick(r, VALS),
    ], pushW);
  ...
  if (op < 10 && isLst(node) && lst(node).length > 0) {
    ...
    if (r() < 0.5) {
      ...
      peer.wrapList(t, tag);
      return `wrapList(${t}, ${tag})`;
    } else {
      const f = pick(r, NAMES), tag = pick(r, TAGS);
      peer.wrapRecord(t, f, tag);
      return `wrapRecord(${t}, ${f}, ${tag})`;
    }
  }
  ...
  if (
    op < 18 && path && (isRec(node) || isLst(node)) &&
    (!wild || allMatch(doc, path, (n) => isRec(n) || isLst(n)))
  ) {
    const tag = pick(r, TAGS);
    peer.updateTag(path, tag);
    return `updateTag(${p}, ${tag})`;
  }
  if (op < 22 && isRec(node) && (!wild || allMatch(doc, path, isRec))) {
    const fs = flds(node);
    if (fs.length === 0) return null;
    const from = pick(r, fs);
    const to = NAMES.filter((n) => n !== from && !fs.includes(n));
    if (to.length === 0) return null;
    const t = pick(r, to);
    peer.rename(path, from, t);
    return `rename(${p}, ${from}→${t})`;
  }
  if (op < 25 && isRec(node) && (!wild || allMatch(doc, path, isRec))) {
    const unused = NAMES.filter((n) => !flds(node).includes(n));
    if (unused.length === 0) return null;
    const f = pick(r, unused), v = pick(r, VALS);
    peer.add(path, f, v);
    return `add(${p}, ${f}=${v})`;
  }
  if (
    op < 27 && isRec(node) && flds(node).length > 1 &&
    (!wild || allMatch(doc, path, isRec))
  ) {
    const f = pick(r, flds(node));
    peer.delete(path, f);
    return `delete(${p}, ${f})`;
  }
  if (op < 29 && !wild && isLst(node) && lst(node).length >= 2) {
    ...
    let dst = int(r, 0, len - 1);
    if (dst === src) dst = (dst + 1) % len;
    peer.copy(`${path}/${dst}`, `${path}/${src}`);
    return `copy(${p}/${dst}, ${p}/${src})`;
  }
  return null;
}
...
for (let seed = 0; seed < seeds; seed++) {
  ...
  const peers = [
    new Denicek("alice", INITIAL_DOC),
    new Denicek("bob", INITIAL_DOC),
    new Denicek("carol", INITIAL_DOC),
  ];
  ...
  for (let iter = 0; iter < iters; iter++) {
    ...
    if (verbose && desc) {
      console.log(
        `  ${iter}: ${names[peerIdx]} ${desc}  → sync ${names[a]}↔${names[b]}`,
      );
    }
