...
type RemoteEditDecoder<TEncodedEdit extends EncodedRemoteEdit = EncodedRemoteEdit> = (encodedEdit: TEncodedEdit) => Edit;

const remoteEditDecoders = new Map<EncodedRemoteEdit['kind'], RemoteEditDecoder>();

export function registerRemoteEditDecoder<TEncodedEdit extends EncodedRemoteEdit>(
...
export function decodeRemoteEdit(encodedEdit: EncodedRemoteEdit): Edit {
  ...
  if (decoder === undefined) {
    throw new Error(`decodeRemoteEdit: unknown edit kind "${encodedEdit.kind}".`);
  }
